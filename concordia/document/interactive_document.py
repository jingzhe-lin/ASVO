...
class InteractiveDocument(document.Document):
  ...
  def open_question(
    ...
    Args:
      ...
      question：要问的问题。
      Forced_response：强制文档提供此响应。LLM将不会被咨询。如果wonse_prefix包含在强制响应中，则将其删除。
      Answer_prefix：添加到模型提示符的前缀。
      Answer_suffix：附加到模型响应的后缀。
      Max_tokens：要从模型中采样的令牌的最大数量。
      terminators：不能出现在模型响应中的字符串。如果由模型发出，则响应将在它们之前被截断。
      question_label：用于问题的标签，通常是“Question”。
      answer_label：用于回答的标签，通常是“Answer”。
    ...
    if response is None:
      raise ValueError(
          'Language model returned None from sample_text after retry; check '
          'model configuration or forced_response handling.'
      )
